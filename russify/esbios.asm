...
;ESBIOS9b - улучшены комментарии в реконфигураторе памяти, 
;добавлено переключение между основной и альт
;кодовыми таблицами по Ctrl-ИНФ.
;Улучшен обработчик int10,ax1103
; - в idnpol пишется идентификатор таблицы (содержимое BL) 
;29.09.2020 (24.09.21 - изменения для UTF-8)
...
;	 	 	    ASCII (USA),
;	 	 	    BL=2 - загрузить вторую половину знакогене-
;	 	 	    ратора из ПЗУ кодовой таблицы с русским
;	 	 	    алфавитом (ISO)
;			    (id таблиц 0,1,2 сохраняется в idnpol)
;
;   На выходе:
;
...
;RCgoff новая программа основана на int10,ah11
	...
	mov	bx,0800h	;8 байт/символ, русская таблица 866
	int	10h
				;после возврата в AL=0(id таблицы), в AH - 08h
...
ah12:
 	...
 	jnz	ah121
 	push	cs              
 	pop	es              ;rc указатель на знакогенератор д.б es:bp
 	mov	cx,80h          ;rc загрузить 128 символов
 	mov	dx,cx	 	;rc начальный код символа 128
 	mov	dh,bl	 	;rc переписали код таблицы в id пользователя
 	mov	bh,0
 	shl	bl,1	 	;rc сформировали указатель на слово
 	mov	bp,cs:[bx+offset charpg]	;rc в bp - нужное смещение знакогенератора
 	mov	bh,08           ;rc 8 байт/символ             	
 	mov	ax,1100h        ;rc команда загрузки произвольного знакогенератора
 	int	10h
 	jmp	video_return
ah121:
 	jmp	znak
charpg:	dw	crt_char_866
 	dw	crt_char_ibm
 	dw	crt_char_iso
...
k307:	mov	ax,0a000h			;rc клавиша ИНФ, расширенный скан-код
 	jmp	inf_rc	 	    ;rc обработаем смену кодовой таблицы, если Ctrl-Инф
...
;RCgoff - в версии 9 здесь, на освобожденном от обработчиков capslock месте, 
;rc размещается обработчик ИНФ.
;rc отлавливается нажатие CTRL-ИНФ
;rc по такому нажатию происходит перезагрузка знакогенератора
;rc т.е. если была таблица 866, ставится основная ГОСТ, и наоборот.
;rc используется подфункция 1103h, модифицированная rc - 
;rc в id пользователя idnpol 40:90 заносится код таблицы.
;rc Эта idnpol используется как флажок для CTRL-ИНФ

inf_rc:
 	push	ax
 	test	kb_flag,ctl_shift
        jz	inf_ret 	 	;rc нет CTRL - обрабатываем ИНФ как обычно
 	mov	ax,idnpol	 	;rc грузим ид табл пользователя
 	xor	ax,0802h	 	;rc должен получиться инвертированный id в al
 	test	ax,0ffdh	 	;rc если биты, кроме 1-го, равны 1, значит id или байт/симв не 0800,0802
 	jnz	inf_ret	 	 	;rc в этом случае выходим, ничего не меняя
 	push	ds                      ;rc если id и байт/симв ок, уст int 1fh на новую таблицу (для граф режимов)
 	push	bx
 	xor	bx,bx
 	mov	ds,bx	 	 	;rc уст ds на табл вект прерыв
 	add	bl,al	 	 	;rc играет роль пересылки, но с установкой флагов (bl=0)


;rc	jz	set866
;rc	mov	ax,offset crt_char_iso

        shl	bx,1
 	mov	ax,cs:[offset charpg+bx]
 	shr	bx,1

p866ret:mov	ds:7ch,ax	 	;rc 7ch суть адрес вектора 1fh, через 1fh*4 не работает
 	pop	ds                      ;rc восст ds на обл данных ПЗУ
 	mov	ax,1103h	 	;rc функция загрузки верхней половины знакогенератора
 	int	10h
 	pop	bx
inf_ret:pop	ax	 	 	;rc обычная обработка ИНФ (0a00 расширенный код уже записан в k307)
 	jmp	k57    	 	 	

;rc set866:	mov	ax,offset crt_char_866
;rc	jmp	short p866ret
